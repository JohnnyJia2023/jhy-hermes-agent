...
# ── process-group cleanup + probe_copilot_acp (ported from jhy/fix) ────

import signal as _signal
import subprocess as _subprocess

from agent import copilot_acp_client as _module
from agent.copilot_acp_client import probe_copilot_acp


class _FakeProcGroup:
...
def test_close_terminates_copilot_process_group(monkeypatch):
    ...
    killpg_calls = []
    monkeypatch.setattr(_module.os, "getpgid", lambda pid: pid + 1)
    monkeypatch.setattr(_module.os, "killpg", lambda pgid, sig: killpg_calls.append((pgid, sig)))

    client.close()

    assert killpg_calls == [(4243, _signal.SIGTERM)]
...
def test_run_prompt_launches_acp_in_its_own_session(monkeypatch):
    ...
    proc = _FakeProcGroup(stdout_lines=responses)
    popen_kwargs = {}
    ...
    killpg_calls = []
    monkeypatch.setattr(_module.subprocess, "Popen", _fake_popen)
    ...
    assert killpg_calls == [(4242, _signal.SIGTERM)]
...
def test_close_escalates_to_sigkill_when_group_does_not_exit(monkeypatch):
    ...
    def _wait(timeout=None):
        ...
        if len(proc.wait_calls) == 1:
            raise _subprocess.TimeoutExpired(cmd="copilot", timeout=timeout)
        proc._returncode = -9
        return 0
    ...
    killpg_calls = []
    monkeypatch.setattr(_module.os, "getpgid", lambda pid: pid + 7)
    monkeypatch.setattr(_module.os, "killpg", lambda pgid, sig: killpg_calls.append((pgid, sig)))

    client.close()

    assert killpg_calls == [
        (4249, _signal.SIGTERM),
        (4249, _signal.SIGKILL),
    ]
